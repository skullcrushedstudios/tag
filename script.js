class MultiplayerTagGame {
  ...
  connectToServer() {
    ...
    this.socket.on("connect", () => {
      ...
      this.connectionStatus.textContent =
        "Connected! Enter your details to join a room.";
      this.connectionStatus.className = "connection-status connected";
    ...
    this.socket.on("playerLeft", (data) => {
      // Clean up remote player interpolation data
      if (data.playerId && this.remotePlayers.has(data.playerId)) {
    ...
    this.socket.on("playerMoved", (data) => {
      ...
      if (data.playerId !== this.playerId) {
        ...
        if (!this.remotePlayers.has(data.playerId)) {
          ...
          remotePlayer.target = { x: data.position.x, y: data.position.y };
        }
      }
    });
  ...
  setupEventListeners() {
    ...
    document
      ...
      .addEventListener("click", () => this.selectOnlineMode());
    document
      .getElementById("offlineMode")
      .addEventListener("click", () => this.selectOfflineMode());
    document
      .getElementById("backToModeBtn")
      .addEventListener("click", () => this.showModeSelection());
    document
      .getElementById("backToModeBtn2")
      .addEventListener("click", () => this.showModeSelection());
    ...
    // Offline mode
    document
      .getElementById("startOfflineBtn")
      .addEventListener("click", () => this.startOfflineGame());
    ...
    document
      .getElementById("backToMainFromLogin")
    ...
    document
      .getElementById("privateServer")
      .addEventListener("change", () => this.toggleServerInputs());
    document
      .getElementById("groupServer")
      .addEventListener("change", () => this.toggleServerInputs());

    // Admin panel
    document
      .getElementById("closeAdminPanel")
      .addEventListener("click", () => this.hideAdminPanel());
    document
      .getElementById("banPlayerBtn")
      .addEventListener("click", () => this.banPlayer());
    document
      .getElementById("viewPlayersBtn")
      .addEventListener("click", () => this.viewAllPlayers());
    document
      .getElementById("kickAllBtn")
      .addEventListener("click", () => this.kickAllPlayers());
    document
      .getElementById("debugModeBtn")
      .addEventListener("click", () => this.toggleDebugMode());
    document
      .getElementById("showStatsBtn")
      .addEventListener("click", () => this.showGameStats());
    document
      .getElementById("resetGameBtn")
      .addEventListener("click", () => this.resetCurrentGame());
    document
      .getElementById("serverInfoBtn")
      .addEventListener("click", () => this.showServerInfo());
    document
      .getElementById("broadcastBtn")
      .addEventListener("click", () => this.broadcastMessage());
    document
      .getElementById("sendMotdBtn")
      .addEventListener("click", () => this.setMessageOfDay());
    document
      .getElementById("executeBtn")
      .addEventListener("click", () => this.executeConsoleCommand());
    document
      ...
      const checkConnection = () => {
        if (this.socket && this.socket.connected) {
          ...
          const actualPlayerName =
            playerName || this.currentUser?.username || "Anonymous";
      ...
      if (this.gameState && this.gameState.isRunning) {
        if (this.isOfflineMode) {
          ...
          if (currentPlayer) {
            ...
            // Send to server at throttled rate with delta compression
            const now = Date.now();
            if (now - this.lastMoveTime >= this.moveThrottle) {
              // Only send if position changed significantly (>1px)
              const deltaX = Math.abs(newPos.x - this.lastSentPos.x);
              const deltaY = Math.abs(newPos.y - this.lastSentPos.y);

              if (deltaX > 1 || deltaY > 1) {
                this.socket.emit("playerMove", newPos);
                this.lastSentPos = { x: newPos.x, y: newPos.y };
              }
  ...
  interpolateRemotePlayers() {
    ...
    for (const [playerId, remoteData] of this.remotePlayers) {
      if (playerId !== this.playerId) {
        const playerElement = this.players.get(playerId);
        if (playerElement && remoteData) {
          // Smooth interpolation toward target position
          remoteData.current.x +=
            (remoteData.target.x - remoteData.current.x) * lerpFactor;
          remoteData.current.y +=
            (remoteData.target.y - remoteData.current.y) * lerpFactor;

          // Update visual position
          playerElement.style.left =
            remoteData.current.x - this.playerSize / 2 + "px";
          playerElement.style.top =
            remoteData.current.y - this.playerSize / 2 + "px";
        }
      }
    }
  }
  ...
  handleOfflineMovement() {
    ...
    if (this.keys["s"]) {
      player1.y = Math.min(
        this.gameHeight - this.playerSize / 2,
        player1.y + p1Speed
      );
    }
    if (this.keys["a"]) {
    ...
    if (this.keys["d"]) {
      player1.x = Math.min(
        this.gameWidth - this.playerSize / 2,
        player1.x + p1Speed
      );
    }
  ...
  checkOfflineTagging() {
    if (this.offlineGameState.tagCooldown || this.offlineGameState.qteActive)
      return;
  ...
  handleOfflineTag() {
    // Switch who is "it"
    this.offlineGameState.players[0].isIt =
      !this.offlineGameState.players[0].isIt;
    this.offlineGameState.players[1].isIt =
      !this.offlineGameState.players[1].isIt;
  ...
  startOfflineQTE() {
    ...
    const qteData = {
      keys: selectedKeys,
      duration: 3000,
      players: this.offlineGameState.players.map((p) => ({
        id: p.id,
        name: p.name,
      })),
    };
  ...
  handleQTEInput(key) {
    ...
    if (key === expectedKey) {
      ...
      const keyElement = document.getElementById(
        `qte-key-${this.qteData.currentKeyIndex}`
      );
      if (keyElement) {
      ...
      if (this.qteData.currentKeyIndex < this.qteData.keys.length) {
        const nextKeyElement = document.getElementById(
          `qte-key-${this.qteData.currentKeyIndex}`
        );
        if (nextKeyElement) {
  ...
  handleOfflineQTEInput(key) {
    ...
    const expectedKey =
      this.qteData.keys[
        Math.min(player1Progress.currentIndex, player2Progress.currentIndex)
      ];
    ...
    if (key === expectedKey) {
      ...
      // Update visual feedback
      const keyElement = document.getElementById(
        `qte-key-${progress.currentIndex - 1}`
      );
      if (keyElement) {
        keyElement.classList.remove("active");
        keyElement.classList.add("completed");
  ...
  calculateNewPosition(currentPos) {
    ...
    const currentPlayer = this.gameState
      ? this.gameState.players.find((p) => p.id === this.playerId)
      : null;
  ...
  showQTEResults(data) {
    ...
    results.innerHTML = `
            <p><strong>Winner:</strong> ${data.winner || "None"} (${
      data.winnerKeys
    } keys)</p>
            <p><strong>Loser:</strong> ${data.loser || "None"} (${
      data.loserKeys
    } keys)</p>
  ...
  viewAllPlayers() {
    ...
    if (this.gameState && this.gameState.players) {
      this.gameState.players.forEach((player, index) => {
        ...
        this.logToConsole(
          `${index + 1}. ${player.name} ${status} ${slowed}`,
          "info"
        );
      });
    } else {
    ...
    this.logToConsole(
      `Total banned players: ${this.bannedPlayers.size}`,
      "warning"
    );
  }
  ...
  kickAllPlayers() {
    if (
      confirm(
        "Are you sure you want to kick all players? This will end the current game."
      )
    ) {
  ...
  showGameStats() {
    ...
    if (this.gameState) {
      this.logToConsole(
        `Time remaining: ${this.gameState.timeLeft || 0} seconds`,
        "info"
      );
      this.logToConsole(`Total tags: ${this.gameState.tagCount || 0}`, "info");
      this.logToConsole(
        `Game running: ${this.gameState.isRunning ? "Yes" : "No"}`,
        "info"
      );
      this.logToConsole(
        `Players: ${
    ...
    this.logToConsole(
      `Connection status: ${
        this.socket && this.socket.connected ? "Connected" : "Disconnected"
      }`,
      "info"
    );
  ...
  showServerInfo() {
    ...
    this.logToConsole(
      `Socket connected: ${
        this.socket && this.socket.connected ? "Yes" : "No"
      }`,
      "info"
    );
  ...
  broadcastMessage() {
    ...
    this.logToConsole(
      `Broadcasting message: "${message}" (${messageType})`,
      "info"
    );
